...
if ($selectedCourseId) {
    ...
    $catStmt = $db->prepare('SELECT fc.*, c.title AS course_title, COUNT(ft.id) AS topic_count
                             FROM forum_categories fc
                             JOIN courses c ON fc.course_id = c.id
                             LEFT JOIN forum_topics ft ON fc.id = ft.category_id
                             WHERE c.instructor_id = ?
                             GROUP BY fc.id
                             ORDER BY c.title, fc.name');
...
if ($selectedCourseId) {
    if ($selectedCategoryId) {
        $topicStmt = $db->prepare('SELECT ft.*, fc.name as category_name, c.title AS course_title,
    ...
    if ($selectedCategoryId) {
        $topicStmt = $db->prepare('SELECT ft.*, fc.name as category_name, c.title AS course_title,
                                   ...
                                   JOIN forum_categories fc ON ft.category_id = fc.id
                                   JOIN courses c ON fc.course_id = c.id
                                   WHERE c.instructor_id = ? AND fc.id = ?
                                   ORDER BY ft.is_pinned DESC, ft.last_post_at DESC');
        $topicStmt->execute([$_SESSION['user_id'], $selectedCategoryId]);
    } else {
        $topicStmt = $db->prepare('SELECT ft.*, fc.name as category_name, c.title AS course_title,
                                          (SELECT COUNT(fp.id) FROM forum_posts fp WHERE fp.topic_id = ft.id) as post_count,
                                          (SELECT id FROM forum_posts fp WHERE fp.topic_id = ft.id ORDER BY fp.created_at ASC LIMIT 1) as first_post_id,
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <?php if ($selectedCourseId): ?>
                    <div class="mb-3 d-flex justify-content-end gap-2">
                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#createCatModal">
                                    ...
                                    <div class="list-group list-group-flush">
                                        <?php foreach ($categories as $cat): ?>
                                            <div class="list-group-item d-flex justify-content-between align-items-start">
                                                <div class="me-auto">
                                                    <div class="fw-bold"><?php echo htmlspecialchars($cat['name']); ?>
                                                        <?php if (isset($cat['course_title'])): ?>
                                                            <span class="badge bg-light text-dark ms-1"><?php echo htmlspecialchars($cat['course_title']); ?></span>
                                                        <?php endif; ?>
                                                    </div>
                                        ...
                                        <table class="table table-hover align-middle">
                                            ...
                                            <tbody>
                                                <?php foreach ($topics as $t): ?>
                                                    <tr>
                                                        <td><span class="badge bg-dark"><?php echo htmlspecialchars($t['course_title'] ?? ''); ?></span></td>
                                                        <td><a href="../forum/topic_view.php?id=<?php echo $t['id']; ?>" class="text-decoration-none fw-bold"><?php echo htmlspecialchars($t['title']); ?></a></td>
                ...
                <div class="modal fade" id="createCatModal" tabindex="-1">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title"><i class="fas fa-folder-plus"></i> New Category</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <form method="post">
                                <div class="modal-body">
